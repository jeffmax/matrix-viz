// ══════════════════════════════════════════════════
// TAB 4 — EMBEDDING BACKWARD: blh,blc→hc (gradient accumulation)
// ══════════════════════════════════════════════════
...
let ebPlaying = false;
let ebTm = null;
let ebSelectedCell = null; // {h, c} for trace-back
...
export function ebInit(randomize = true) {
  if (randomize) {
    ...
    G = Array.from({ length: eB }, () => Array.from({ length: eL }, () => Array(eC).fill(1)));
  }
...
function resetAccum() {
  dWAccum = Array.from({ length: eH }, () => Array(eC).fill(0));
}
...
export function ebRender() {
  ...
  if (ebStep >= 0) {
    ...
    for (let h = 0; h < eH; h++) {
      ...
      html += `<div class="ef-w-rowlabel${isActive ? ' active' : ''}">h=${h}</div>`;
      for (let c = 0; c < eC; c++) {
        const val = X[b][l][h] * G[b][l][c];
  ...
  html += '<div class="eb-section">';
  html += '<div class="ef-section-label">dW accumulator <span class="ef-dim">(H=' + eH + ', C=' + eC + ')</span></div>';
  ...
  for (let h = 0; h < eH; h++) {
    const isActive = h === activeRow && ebStep >= 0;
    html += `<div class="ef-w-rowlabel${isActive ? ' active' : ''}">h=${h}</div>`;
    for (let c = 0; c < eC; c++) {
      const val = dWAccum[h][c];
      ...
      html += `<div class="mat-cell r${isFinal ? ' done' : ''}" `
        ...
        + `onclick="ebTraceBack(${h},${c})">${val}</div>`;
    }
  }
  html += '</div></div>';
...
function ebUpdateFormula() {
  ...
  if (ebSelectedCell) {
    const { h, c } = ebSelectedCell;
    // Show all positions that contribute to this cell
    let terms = [];
    for (let b = 0; b < eB; b++) {
      for (let l = 0; l < eL; l++) {
        if (tokenIds[b][l] === h) {
          terms.push(`<span class="fa">G[${b},${l},${c}]</span>=${G[b][l][c]}`);
        }
      }
    }
    if (terms.length === 0) {
      f.innerHTML = `dW[${h},${c}] = 0 — <em style="color:#999">no tokens had id=${h}</em>`;
    } else {
      f.innerHTML = `dW[${h},${c}] = ${terms.join(' + ')} = <span class="fc">${dW[h][c]}</span>`
        + ` <em style="color:#999">— sum of gradients at positions where token=${h}</em>`;
    }
    return;
  }

  if (ebStep < 0) {
    f.innerHTML = `Backward pass: <code>dW[h,c] = Σ<sub>b,l</sub> X[b,l,h]·G[b,l,c]</code>. `
...
// ── Trace-back ──
export function ebTraceBack(h, c) {
  if (ebSelectedCell && ebSelectedCell.h === h && ebSelectedCell.c === c) {
    ebSelectedCell = null;
  } else {
    ebSelectedCell = { h, c };
  }
...
export function getEbState() {
  return { eB, eL, eH, eC, tokenIds, X, G, dW, dWAccum, ebStep, ebPlaying, ebSelectedCell };
}
